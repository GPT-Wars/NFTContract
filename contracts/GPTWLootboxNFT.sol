...
contract GPTWarsLootboxNFT is ERC721URIStorage, Ownable {
    ...
    mapping(uint => Lootbox) public lootboxTypes;
    mapping(address => uint) public lootboxBalance;
    mapping(uint => Cosmetic) public tokenIdToCosmetic;
    ...
    function createNFT(address recipient, string memory metadataURI) public onlyOwner {
        ...
        _setTokenURI(nextTokenId, metadataURI);
        userNFTs[recipient].push(nextTokenId);
        tokenIdToCosmetic[nextTokenId] = Cosmetic("Custom Cosmetic", "Unknown", 0, metadataURI);
        emit NFTCreated(recipient, nextTokenId, metadataURI);
        nextTokenId++;
    }
    ...
    function openLootbox(uint lootboxlevel) public {
        ...
        _mint(msg.sender, nextTokenId);
        _setTokenURI(nextTokenId, selectedCosmetic.metadataURI);
        tokenIdToCosmetic[nextTokenId] = selectedCosmetic;
        userNFTs[msg.sender].push(nextTokenId);
    ...
    function openOneLootbox(string memory name) public {
        ...
        uint lootboxLevel = _getLootboxLevelByName(name);
        require(lootboxLevel <= 5, "Invalid lootbox name"); // Autorise level 0 aussi !
    ...
    function _getLootboxLevelByName(string memory name) internal view returns (uint) {
        ...
        revert("Lootbox not found");
    }

    function _selectCosmetic(uint lootboxlevel) internal view returns (Cosmetic memory) {
        require(cosmeticSets[lootboxlevel].length > 0, "No cosmetics available");
        uint random = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nextTokenId))) % cosmeticSets[lootboxlevel].length;
        return cosmeticSets[lootboxlevel][random];
    }

    function getCosmeticByTokenId(uint tokenId) public view returns (Cosmetic memory) {
        require(_exists(tokenId), "Token does not exist");
        return tokenIdToCosmetic[tokenId];
    }
    ...
    function getLootboxNFTs(address user) public view returns (Cosmetic[] memory) {
        uint[] memory tokenIds = userNFTs[user];
        Cosmetic[] memory cosmetics = new Cosmetic[](tokenIds.length);

        for (uint i = 0; i < tokenIds.length; i++) {
            cosmetics[i] = tokenIdToCosmetic[tokenIds[i]];
        }
        return cosmetics;
    }
    ...
    function _removeNFT(address user, uint tokenId) internal {
        uint[] storage tokens = userNFTs[user];
    ...
    function evolveNFT(uint tokenId1, uint tokenId2, uint tokenId3) public {
        require(ownerOf(tokenId1) == msg.sender, "Not owner of tokenId1");
        require(ownerOf(tokenId2) == msg.sender, "Not owner of tokenId2");
        require(ownerOf(tokenId3) == msg.sender, "Not owner of tokenId3");

        _burn(tokenId1);
        _burn(tokenId2);
        _burn(tokenId3);

        _removeNFT(msg.sender, tokenId1);
        _removeNFT(msg.sender, tokenId2);
        _removeNFT(msg.sender, tokenId3);

        string memory evolvedMetadata = "ipfs://new-evolved-nft-metadata-uri";
        _mint(msg.sender, nextTokenId);
        _setTokenURI(nextTokenId, evolvedMetadata);

        Cosmetic memory evolvedCosmetic = Cosmetic("Evolved Cosmetic", "Special", 6, evolvedMetadata);
        tokenIdToCosmetic[nextTokenId] = evolvedCosmetic;
        userNFTs[msg.sender].push(nextTokenId);

        emit NFTEvolved(nextTokenId, tokenId1, tokenId2, tokenId3);
